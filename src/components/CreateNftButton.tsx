...
const CreateNftButton = ({ onCreated }: { onCreated?: () => void }) => {
  ...
  const create = async () => {
    ...
    try {
      ...
      setStep("Generating your server…");
      ...
      toast({ title: "Server created!", description: "Your custom server is ready" });
      onCreated?.();
    } catch (err) {
    ...
    >
      <DialogTrigger asChild>
        <Button className="h-12 w-full rounded-2xl font-display text-sm glow-primary gap-2">
          Create your Server · 4 Gram
        </Button>
      </DialogTrigger>
      <DialogContent className="rounded-3xl">
        <DialogHeader>
          <DialogTitle className="font-display">Create your Server</DialogTitle>
          <DialogDescription>
        ...
        {result ? (
          <div className="space-y-3">
            <img src={result} alt="Your generated server" className="w-full rounded-2xl"  loading="lazy" decoding="async" />
